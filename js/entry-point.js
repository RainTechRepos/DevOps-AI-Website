/* ========================================
   js/entry-point.js — Journey Entry Point (FR-W21)
   DevOps AI by RainTech

   Full-screen role selection experience.
   3D Force Graph (desktop with WebGL) + Card Grid (fallback/mobile).
...
(function () {
  ...
  var ZONE_LABELS = {
    ...
    'service-desk': 'Service Desk', 'endpoint-management': 'Endpoint Mgmt',
    'network-ops': 'Network Ops', 'security-operations': 'Security Ops',
  ...
  var ZONE_LIST = [
    ...
    { id: 'endpoint-management', label: 'Endpoint Mgmt' }, { id: 'network-ops', label: 'Network Ops' },
    { id: 'security-operations', label: 'Security Ops' }, { id: 'grc-compliance', label: 'GRC & Compliance' },
  ...
  function buildEntryPointDOM() {
    ...
    container.innerHTML =
      ...
      '<div class="ep-grid-container" id="ep-grid-container">' +
        '<div class="ep-category-tabs" id="ep-category-tabs" role="tablist" aria-label="Role categories"></div>' +
  ...
  function buildCategoryTabs() {
    var tabsEl = document.getElementById('ep-category-tabs');
    if (!tabsEl) return;
    ...
    allTab.className = 'ep-tab ep-tab--active';
    allTab.setAttribute('role', 'tab');
    allTab.setAttribute('aria-selected', 'true');
    allTab.setAttribute('data-group', 'all');
    allTab.textContent = 'All';
    allTab.addEventListener('click', function () { filterByGroup('all'); });
    ...
    groupOrder.forEach(function (groupId) {
      ...
      tab.setAttribute('data-group', groupId);
      tab.innerHTML = '<span class="ep-tab__dot" style="background:' + group.color + '"></span>' + group.label;
      tab.addEventListener('click', function () { filterByGroup(groupId); });
  ...
  function filterByGroup(groupId) {
    ...
    document.querySelectorAll('.ep-tab').forEach(function (tab) {
      var isActive = tab.getAttribute('data-group') === groupId;
      tab.classList.toggle('ep-tab--active', isActive);
      tab.setAttribute('aria-selected', isActive ? 'true' : 'false');
    });

    // Show/hide groups
    document.querySelectorAll('.ep-group').forEach(function (group) {
      if (groupId === 'all') {
        group.style.display = '';
      } else {
        group.style.display = group.getAttribute('data-group-id') === groupId ? '' : 'none';
      }
    });
  }
    ...
    groupOrder.forEach(function (groupId) {
      ...
      section.innerHTML =
        '<div class="ep-group__header">' +
          ...
          '<span class="ep-group__count">' + roles.length + ' roles</span>' +
        '</div>' +
  ...
  // ─── Neural Canvas Background (2D fallback) ───
  function initNeuralBackground() {
    ...
    for (var a = 0; a < nodes.length; a++) {
      for (var b = a + 1; b < nodes.length; b++) {
        ...
        if (Math.sqrt(dx * dx + dy * dy) < maxDist) connections.push({ a: a, b: b });
      }
    }
  ...
  async function init3DGraph() {
    ...
    try {
      ...
      config.roles.forEach(function (role) {
        ...
        (role.zones || []).forEach(function (zoneId) {
      ...
      for (var p = 0; p < particleCount; p++) {
        ...
        scene.add(pmesh);
        particlesList.push({ mesh: pmesh, link: link, t: Math.random(), speed: 0.002 + Math.random() * 0.003 });
      }
      ...
      function animate3d() {
        ...
        particlesList.forEach(function (pt) {
      ...
      particlesList.forEach(function (pt, i) {
  ...
  function initEntryPoint(config) {
    ...
    if (skipBtn) {
      skipBtn.addEventListener('click', function () {
        var epContainer = document.getElementById('devopsai-entry-point');
        if (epContainer) {
          epContainer.classList.add('is-hidden');
          setTimeout(function () { epContainer.remove(); }, 600);
        }
      });
    }
